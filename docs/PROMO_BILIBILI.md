...
![AI 助手演示](screenshots/ai-panel.mp4)
...
① 从 **信号订阅** 拖出信号节点（如 ClassIsland 的 OnClass）。

② 从 **Everything 搜索** 拖出 `search` 节点。

③ 橙色触发线：信号节点的 `o-trigger` → search 的 `i-trigger`。灰色数据线：信号输出的 `Subject` → search 的 `query`。

> 注意看——触发走橙色，数据走灰色。谁先谁后一眼就知道。

④ 从 **消息提醒** 拖出 `ShowMsg` 节点，橙色触发线从 search 连过去。

⑤ 数据线：search 的 `files` → ShowMsg 的 `msgContext`。
...
docs/screenshots/
  main.png              — 完整界面
  sidebar.png           — 侧边栏特写
  node-detail.png       — 单个节点特写（标注触发栏+数据口）
  trigger-vs-data.png   — 同一画布上的橙色虚线和灰色实线
  node-connect.png      — 两个节点连线特写
  code-panel.png        — 生成的代码面板
  ai-panel.png          — AI 面板输入+画布自动生成结果
  demo1-step1.png       — 拖出 pick 节点
  demo1-step3.png       — pick→EdgeTTS 连线
  demo2-full.png        — 信号触发流程（橙色触发+灰色数据）
  funclist-dir.png      — exe 旁边的 funclist 目录
--> 
